...
import { Negociacion, NegociacionDocument } from './schemas/negociacion.schema';
...
export class RequestsService {
  ...
  async registrarRespuestaProveedor(
    ...
    const negociacion = await this.negociacionModel
      .findByIdAndUpdate(
        negociacionId,
        {
    ...
    if (aceptada) {
      await this.actualizarEstado(
        (negociacion as any).solicitudProteccionId.toString(),
        'Finalizada',
      );
    }
